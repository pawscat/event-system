...
import { ParticipantEventFilter } from './participant-event-filter'
import { ParticipantStatusFilter } from './participant-status-filter'

export const dynamic = 'force-dynamic'

export default async function ParticipantsPage(props: { searchParams: Promise<{ eventId?: string, status?: string }> }) {
  const searchParams = await props.searchParams;
  const { eventId, status } = searchParams;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2">
          <a href={`/api/v1/admin/participants/export${eventId ? `?eventId=${eventId}` : ''}`} className="px-4 py-2 border border-border-light bg-surface-container-lowest text-text-main font-label-md text-[14px] font-semibold rounded-lg hover:bg-surface-container-low transition-colors flex items-center gap-2">
            <span className="material-symbols-outlined text-sm">download</span>
            Ekspor
          </a>
          <Link href="/dashboard/admin-registrasi/participants/import" className="px-4 py-2 border border-border-light bg-surface-container-lowest text-text-main font-label-md text-[14px] font-semibold rounded-lg hover:bg-surface-container-low transition-colors flex items-center gap-2">
            <span className="material-symbols-outlined text-sm">upload_file</span>
            Import
          </Link>
          <Link href="/dashboard/admin-registrasi/participants/new" className="px-4 py-2 bg-secondary text-on-secondary font-label-md text-[14px] font-semibold rounded-lg hover:bg-secondary/90 transition-colors shadow-sm flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        {/* Filter Event */}
        <div className="bg-surface-container-lowest p-4 rounded-xl border border-border-light shadow-sm">
          <label className="text-label-sm text-[12px] font-medium text-text-muted block mb-1">Acara</label>
          <ParticipantEventFilter events={events || []} currentEventId={eventId} />
        </div>
      </div>
                  ...
                  <tr key={participant.id} className="border-b border-border-light hover:bg-surface-container-low/50 transition-colors group">
                    ...
                    <td className="py-3 px-4 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <Link href={`/dashboard/admin-registrasi/participants/${participant.id}/edit`} className="p-1.5 text-text-muted hover:text-primary hover:bg-primary/10 rounded transition-colors" title="Edit">
