...
export function ParticipantForm({ initialData, participantId, eventId }: { initialData?: Partial<ParticipantData>, participantId?: string, eventId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/dashboard/super-admin/participants')
      router.refresh()
    } catch (err: unknown) {
    ...
    <form onSubmit={handleSubmit} className="bg-surface-container-lowest border border-border-light rounded-xl shadow-sm p-6 max-w-2xl">
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-end gap-3 pt-6">
          <Link href={`/dashboard/super-admin/events/${eventId}/participants`} className="px-6 py-2.5 rounded-lg border border-border-light bg-surface text-text-main font-label-md text-[14px] font-semibold hover:bg-surface-container-low transition-colors">
            Batal
          </Link>
          <button 
            type="submit" 
